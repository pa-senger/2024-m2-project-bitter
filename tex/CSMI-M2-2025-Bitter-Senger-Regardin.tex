...
After running the steady case simulation, we can visualize the results using Paraview \cite{paraview}.
...
We can see that the bigger hole is not effective to cool down the magnet, whereas
the smaller ones are better.
According to the boundary settings for heat transfer (Robin for the holes
  $\Gamma_{\text{cool},2}$ and sides $\Gamma_{\text{channel}}$ and Neumann homogeneous
for the hole $\Gamma_{\text{cool},1}$), this is a situation we were expecting to observe
...
\begin{enumerate}
  ...
  \item Run and analyze transient cases.
\end{enumerate}
