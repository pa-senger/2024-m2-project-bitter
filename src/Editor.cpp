...
vtkContourGrid *Editor::extractIsoSurfaceVTK(const double &isoValue)
{
    ...
    // Ensure that input data exists
    if (!inputMesh_)
    {
        std::cerr << "Error: No data set available for iso-surface extraction."
                  << std::endl;
        return nullptr;
    }

    // Create a vtkContourGrid instance
    vtkContourGrid *contour = vtkContourGrid::New();
    contour->SetInputData(inputMesh_); // Set the input dataset
    contour->SetValue(0, isoValue);    // Set the iso-value

    // Update the pipeline to generate the iso-surface
    contour->Update();

    std::cout << "[Editor] VTK iso-surface extraction done in "
              << t.getElapsedTime() << " s." << std::endl;

    // Return the vtkContourGrid instance
    return contour;
}
                           ...
                           const bool &useVtk)
{
    ...
    cout << "[Editor] Moving isosurface #" << isoSurfaceId << " to "
         << newIsoValue << " from range [" << range[0] << ";" << range[1]
         << "] ..." << endl;
