...
UserInterface::~UserInterface()
{
    ...
    if (mainActor_)
        mainActor_->Delete();
    if (geometryFilter_)
        geometryFilter_->Delete();
    if (boundaryMapper_)
        boundaryMapper_->Delete();
    if (renderer_)
        renderer_->Delete();
    if (renderWindow_)
        renderWindow_->Delete();
    if (customInteractor_)
        customInteractor_->Delete();
    if (interactor_)
        interactor_->Delete();
...
int UserInterface::addIsoSurface()
{
    ...
    cout << "[UserInterface] Adding an isosurface at isovalue "
         << (range[0] + range[1]) / 2 << " from range [" << range[0] << ";"
         << range[1] << "] ..." << endl;
...
int UserInterface::switchCurrentIsoSurfaceTransparency()
{
    ...
    {
        isoSurfaceActors_[currentIsoSurfaceId_]->GetProperty()->SetOpacity(
            ISOSURFACE_ALPHA);
