...
class VTK_EXPORT IsoSurfacer : public vtkAlgorithm
{
  ...
  protected:
        ...
        pair<vtkIdType, vtkIdType> &edge) const
    {
        ...
        vtkIdType id1 = edge.first;  // first point of the edge
        vtkIdType id2 = edge.second; // second point of the edge

        // get the scalar values at the two points
        double scalar1 = scalarField_->GetTuple(id1)[0];
        double scalar2 = scalarField_->GetTuple(id2)[0];

        // compute the intersection point
        double t = (Value - scalar1) / (scalar2 - scalar1); // interpolation

        p[0] = (1 - t) * pointSet_->GetPoint(id1)[0] +
               t * pointSet_->GetPoint(id2)[0];
        p[1] = (1 - t) * pointSet_->GetPoint(id1)[1] +
               t * pointSet_->GetPoint(id2)[1];
        p[2] = (1 - t) * pointSet_->GetPoint(id1)[2] +
               t * pointSet_->GetPoint(id2)[2];
    ...
    inline bool IsCellOnLevelSet(vtkCell *cell) const
    {
        ...
        std::cout << "Number of points in the cell = "
                  << cell->GetNumberOfPoints() << std::endl;
